...
CHOOSING generate_image vs generate_broll for b-roll requests:
- If the b-roll needs to show LEGIBLE TEXT — screen recordings of code, app UIs with buttons/labels, slides with titles, signs with words, infographics, chat screenshots — call generate_image. Text-to-video models hallucinate gibberish glyphs in place of real text; image models like gpt-image-2 render text accurately.
- For "match the vibe" requests, the deterministic signal is analyze_clip's \`hasOnScreenText\` field — when true, route to the image-then-animate chain (see "DEFAULT BEHAVIOR — chain analyze_clip + generation" above). Do not second-guess hasOnScreenText with prose-keyword matching against visualDescription.
- For everything else (people, scenery, products, abstract footage, motion that doesn't depend on legible text) — keep using generate_broll.
- When the user asks for "a still" / "a screenshot" / "an image of …" — that's always generate_image, never generate_broll.
