...
- add_kinetic_title({ title, subtitle?, accent_color?, start_seconds, target_seconds? }): render a STYLIZED, ANIMATED title card LOCALLY (free, no API, no cloud) via the HyperFrames engine and drop it on the timeline. The words stagger in with a blur-clear, an accent underline wipes open, an optional subtitle fades up. Use when the user wants a FANCY / "motion-designed" / "cinematic" / "animated" title with effects beyond plain text. This is a RENDERED clip: a placeholder appears immediately and the finished clip swaps in after ~10-30s (one Ctrl+Z removes it), same as generate_broll. CHOOSING between this and add_motion_graphic template:"title_card": prefer add_motion_graphic (instant, hand-editable text) when the user will TWEAK the title or wants it now; use add_kinetic_title when they explicitly want the richer animated/stylized look and accept a short render + a baked (not text-editable) clip. Default target_seconds is 4. start_seconds defaults to the playhead conceptually — pass the playhead position.
